...
st.set_page_config(page_title="Tender SQM Mapping Wizard v13.4", layout="wide")
st.title("Tender SQM Mapping Wizard – v13.4 (Excel-style preview)")
...
# Excel-style preview of raw sheet
st.markdown("#### Preview of raw sheet (first 30 rows, Excel-style)")

rows_preview = min(30, len(df_raw))
preview_df = df_raw.head(rows_preview).copy()
preview_df.index = range(1, rows_preview + 1)
...
excel_cols = [_num_to_col(i + 1) for i in range(df_raw.shape[1])]
preview_df.columns = excel_cols
